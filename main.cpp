...
int main(int argc, char *argv[])
{
    try {
        bool start = false;
        int port = 33333;
        string basa = "/home/stud/Kurs-main/Code/base.txt";
        string jur = "/home/stud/Kurs-main/Code/error.txt";
            ...
            while(true) {
                ...
                if(work_sock == -1) {
                    ...
                    for (uint32_t i = 0; i < num_v; ++i) {
                        ...
                        uint64_t* data = new uint64_t[v_len]; 
                        rc = recv(work_sock, (void *) data, sizeof(uint64_t) * v_len, 0); 
                        if (rc == -1) {
                            vork.errrecord("Некритическая","Ошибка получения значений вектора");
                            close(work_sock);
                            continue;
                        }
                        uint64_t sum = vork.count(v_len,data); 
                        delete[] data;
                        rc = send(work_sock, (void*)&sum, sizeof(uint64_t), 0); 
                        if (rc == -1) {
