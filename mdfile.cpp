...
bool multiplyWithOverflowCheck(uint64_t a, uint64_t b, uint64_t& result) {
...
uint64_t WorkWithClient::count(const uint32_t len, const uint64_t* data)
{
    ...
    for (uint32_t i = 0; i < len; ++i) {
    	...
    	if (!multiplyWithOverflowCheck(rez, data[i], temp_result)) {
        	rez = temp_result;
        /*if (rez * data[i] == (uint64_t)rez * (uint64_t)data[i]) {
            rez *= data[i];*/
        } else {
            if ((data[i] > 0 and rez > 0)) { //or (data[i] < 0 and rez < 0)
                rez = std::numeric_limits<uint64_t>::max();
            } else {
                rez = std::numeric_limits<uint64_t>::min();
            }
        }
    }
    return rez;
